...
def main(config, debug=False, ensemble=None, mlm=False, pseudo_labeling=None, dirpath=None, ensemble_method="Nelder-Mead", ensemble_loss="f1", ensemble_threshold_search=False):
    if dirpath is None:
        dirpath = "exp/"
        if debug: dirpath += "d"
...
if __name__ == "__main__":
    ...
    config = json.load(f)
    f.close()
    main(config, args.d, args.e, args.m, args.p, None, args.em, args.el, args.et)